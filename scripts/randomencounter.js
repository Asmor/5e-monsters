...
define([
	...
	function getBestMonster(targetExp, filters) {
		...
		for ( i = 1; i < miscLib.crList.length; i++ ) {
			if ( miscLib.crList[i].exp < targetExp ) {
				bestBelow = i;
			} else {
				bestAbove = i;
				break;
		...
		if ( (targetExp - miscLib.crList[bestBelow].exp) < (miscLib.crList[bestAbove].exp - targetExp) ) {
			crIndex = bestBelow;
		} else {
			crIndex = bestAbove;
		}
		...
		monsterList = miscLib.getShuffledMonsterList(miscLib.crList[crIndex].string);
	...
	return function (playerCount, partyLevel, filters) {
